...
class FasterRcnn:
    def __init__(self, train_dataset_path, eval_dataset_path, batch, epoch):
        self.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        # 데이터셋 경로 지정
        self.train_dataset_path = train_dataset_path # 학습 데이터 경로
        self.eval_dataset_path = eval_dataset_path # 평가 데이터 경로
